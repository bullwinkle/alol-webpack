@Iconto.module 'office.views.offers', (Offers) ->
	...
	class Offers.CouponEditView extends Marionette.ItemView
		className: 'coupon-edit-view mobile-layout'
		template: JST['office/templates/offers/coupons/coupon-edit']
		...
		initialize: =>
			...
			@state.set
				...
				topbarSubtitle: if @model.isNew() then 'Новая акция' else '&nbsp;'
		...
		onTopbarRightButtonClick: =>
			...
			.then =>
				if isNew
					...
					Iconto.office.router.navigate "office/#{@state.get('companyId')}/offers/coupon/#{@model.get('id')}", trigger: false, replace: true
			.catch (error) =>
				console.error error

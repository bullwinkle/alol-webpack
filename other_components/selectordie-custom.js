            ...
            populateSoD: function ($option, $sodList, $sod, $isPlaceholder) {
                var $sodPlaceholder       = $sod.data("placeholder"),
                    ...
                    $optionIsSelected     = $option.is(":selected") ? " selected active " : "",
		                $optionHasSelected    = $option.attr("selected") !== undefined,
                    $optionIsFirstChild   = $option.is(":first-child"),
                ...
                if ( $option.is("option") ) { // If <option>
                    $("<span/>", {
                        "class":      "sod_option " + $optionCustomClass + $optionIsDisabled + ( $optionIsSelected && ( $optionHasSelected || !$optionIsFirstChild ) ) + $optionLink + $optionLinkExternal,
                        id:           $optionCustomId,
                        title:        $optionText,
                        html:         $optionText,
                    ...
                    else if ( $optionIsSelected && ( $optionHasSelected || !$optionIsFirstChild ) ) { // If the option is selected
                        $sod.data("label", $optionText);
                    }

                    // If selected and no placeholder is set, update label,
                    // added in v.0.1.8: if the placeholder-option is set we'll update the label
                    if ( $optionIsSelected && ( $optionHasSelected || !$optionIsFirstChild ) && ( $optionIsSelected && !$sodPlaceholder || $optionIsSelected && $sodPlaceholderOption || $optionIsSelected && $sodPrefix ) ) {
...
//                        $sod.find(".sod_label").get(0).lastChild.nodeValue = $optionActive.text();
	                      $sod.find(".sod_label").text($optionActive.text());
                    }
                }
            }, // triggerSod
...
//                        $sodLabel.get(0).lastChild.nodeValue = $sodFilterHit.text();
	                      $sodLabel.text($sodFilterHit.text());
...
//                $sod.find(".sod_label").get(0).lastChild.nodeValue = $optionText;
	              $sod.find(".sod_label").text($optionText);
                $sod.data("label", $optionText);
...
//                        $sod.find(".sod_label").get(0).lastChild.nodeValue = $optionSelected.text();
	                      $sod.find(".sod_label").text($optionSelected.text());
                    } else if ( !$optionHasChanged ) {
//                        $sod.find(".sod_label").get(0).lastChild.nodeValue = $sodLabel;
	                      $sod.find(".sod_label").text($sodLabel);
                    }
...
//                        $sod.find(".sod_label").get(0).lastChild.nodeValue = "";
	        ...
	        select: function (value) {
		        return this.each(function () {
			        var $select = $(this),
					        $sod = $select.parent(),
					        $sodSelectedOption = $sod.find(".sod_option[data-value=" + value + "]"),
					        $optionIndex    = $sod.find(".sod_option:not('.optgroup')").index($sodSelectedOption);

			        // Check for the SoD
			        if ($sod.hasClass("sod_select")) {
				        // Check for the option availability
				        if ($sodSelectedOption.length > 0) {
					        $sod.find(".selected, .sod_placeholder").removeClass("selected sod_placeholder");
				          $sod.data("label", $sodSelectedOption.text());
					        $sod.find(".sod_label").text($sodSelectedOption.text());
					        $sodSelectedOption.addClass("selected");
				        ...
				        console.log("Select or Die: There's no SoD to select");
			        }
		        });
	        }, // select
